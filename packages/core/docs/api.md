...
| `avoidLoneSyllables` | boolean | **true** | Keep an automatic line break from leaving one syllable of a word alone at the end of a line (`kö-`) or at the start of the next (`-tek`). A break inside a word is fine when at least two syllables stay on each side. When the greedy fill leaves a lone syllable, the nearby breaks are tried: taking more of the word onto the row (condensing it) or moving the word down (stretching the row it leaves). The alternative that bends the spacing least wins; if none fits the limits below, or moving the word down would cost an extra line, the lone syllable stays. Every stanza counts. Manual `(z)`/`(Z)` breaks are never moved. Set false for the plain greedy breaks. |
| `gapOutlierThresholdMin` | number (staff-spaces) | **1** | Lowest `gapOutlierThreshold` the line breaker may use for a row it condenses to avoid a lone syllable — accepting less even gaps before narrowing any. Clamped to at most `gapOutlierThreshold`. |
| `wrapCondenseMin` | number (fraction) | **0.75** | When a row must be narrowed to keep a syllable company, the share of the natural white space between neumes each gap keeps. Gaps are never narrowed below what their lyrics need. `1` turns narrowing off. |
| `wrapStretchMax` | number | **2** | When a word is moved to the next line, the most extra white space each gap of the row it leaves may take, as a multiple of the natural white space between neumes. |
| `recitationLoneWordMin` | number (em of `lyricSize`) | **2.25** | With `avoidLoneSyllables`, a word of a wrapping tenor recitation at least this wide may stand alone at a line break (`mert Krisztus \| halála lett`). A narrower word (`lett`, `és`) is kept off the line edge by moving words of the phrase to the next line, even when that costs a line. |
| `staffGap` | number (staff-spaces) | **2.5** | Vertical gap between successive staff systems. |
| `lyricDistance` | number (staff-spaces) | **0.5** | Gap between the music's lowest ink and the top of the lyric letters. Because both ends are measured, this is the gap that actually appears on the page: where a virga stem hangs over a syllable, the letters clear its tip by this much and no less. Set against `lyricMinStaffDistance`, so a row with something hanging below the staff is not given less air than a row with nothing. The ink is the drawn ink — virga stems, morae, a below-set ictus — and the letters are measured by their real ascent, so a row of short lower-case syllables rides closer than one carrying capitals. Negative values pull lyrics closer to or inside the staff. |
| `lyricMinStaffDistance` | number (staff-spaces) | **0.75** | Lower bound on how far below the bottom staff line the lyric line sits. Keeps lyrics from crowding the staff when notes are high; ink that descends below the staff still pushes lyrics further down. |
| `lyricLineSkip` | number | **1.2** | Advance from one stanza's lyric line to the next, as a multiple of `lyricSize`. |
| `lyricHyphenMinLen` | number | **0.17** | Shortest a hyphen stroke may be drawn, as a fraction of `lyricSize`. Below this (plus the air on either side) there is no hyphen to draw: the two syllables are pulled into one word instead — with the Hungarian doubled digraph repaired, so `asz-szony` becomes `asszony` — unless the hyphen is mandatory (`=`). |
| `lyricHyphenMaxLen` | number | **0.33** | Longest a hyphen stroke may be drawn, as a fraction of `lyricSize`. |
| `lyricHyphenWidth` | number | **0.04** | Thickness of the hyphen stroke, as a fraction of `lyricSize`. |
| `lyricHyphenSpace` | number | **0.05** | Air on either side of the stroke keeping it off the letters, as a fraction of `lyricSize`. |
| `lyricHyphenPos` | number (x-heights) | **0.55** | Height of the stroke above the lyric baseline, in x-heights of the lyric face — `0.5` puts it across the middle of the letters it stands between, `1` on top of them. Reckoned in x-heights rather than in the body so the stroke sits in the same place in any face. |
| `lyricHyphenRepeat` | number | **4** | A gap wider than this many `lyricSize`s carries several strokes for the one hyphen, so a justified row or a long melisma does not leave a lone hyphen adrift. The white is even: the air before the first stroke, between each pair, and after the last are all the same. |
